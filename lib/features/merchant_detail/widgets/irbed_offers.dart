...
import 'package:jorden_job_task/commons/common_widgets/reuseavle_coupon_card.dart';
...
class IrbedOffers extends ConsumerStatefulWidget {
  const IrbedOffers({super.key, required this.merchantModel});
  final MerchantModel merchantModel;

  @override
  ConsumerState<IrbedOffers> createState() => _AmmanOffersState();
}

class _AmmanOffersState extends ConsumerState<IrbedOffers> {
    ...
    return ExpansionTile(
      ...
      children: [
        Consumer(builder: (context, ref, child) {
          final offerStream =
              ref.watch(watchIrbedOfferProvider(widget.merchantModel));
          return offerStream.when(data: (offer) {
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                return ReuseableCouponCard(offerModel: offer[index]);
              },
            );
          }, loading: () {
            return const Loader();
